...
renderer_dx12_create() {
        ...
        REFIID ref_id = &IID_ID3D12Device;
        HRESULT res = D3D12CreateDevice(0, fl, ref_id, pp_dev);
        assert(!FAILED(res));
        ...
        REFIID ref_id = &IID_ID3D12CommandQueue;
        res = D3D12CreateCommandQueue(dev, &cmd_queue_desc, ref_id, pp_cmd);
       
        assert(!FAILED(res));
                
        IDXGIFactory4 *factory = 0;
        void **pp_fac = (void**)&factory;
        REFIID ref_id = &IID_IDXGIFactory4;
